...
// document.getElementById('btn-validate-plan').addEventListener('click', function () {
//     if (!confirm(ispag_validation.confirmMessage + ' ?')) return;
...
//     .then(r => r.json())
//     .then(res => {
//         console.log('AJAX response:', res); // 👈 ici le log
//         if (res.success) {
//             alert(ispag_validation.drawingValidatedMessage + ' !');
//             if (window.opener) {
//                 window.opener.location.reload(); // recharge la page parente
//                 window.close(); // ferme la popup actuelle
//             } else {
//                 location.reload(); // fallback si pas ouverte via window.open
//             }
//         } else {
//             alert('Erreur : ' + res.data);
//             btn.disabled = false;
